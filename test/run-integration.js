...
const indexName = `health_facility${process.env.TRAVIS_BUILD_NUMBER || 'DEV'}${chance.word({
  length: 12,
})}`;

const client = cliniasearch(appId, apiKey, {
  protocol: 'https:',
  _useCache: false,
});
...
function initPlaces(placesAppId, placesApiKey) {
  return function(t) {
    t.plan(2);
    const places = cliniasearch.initPlaces(placesAppId, placesApiKey);
...
function proxyHttpToHttps(t) {
  ...
  server.listen(0, function() {
    ...
    const proxyClient = cliniasearch(appId, apiKey, {
...
function dnsFailThenSuccess(t) {
  ...
  const client_ = cliniasearch(appId, apiKey, {
