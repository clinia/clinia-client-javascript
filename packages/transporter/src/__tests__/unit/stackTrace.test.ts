...
import { createNullLogger, Logger } from '@clinia/logger-common';
...
describe('transporter stack trace serialization', () => {
  it('removes credentials', async () => {
    const transporterStackTrace = [
      {
        ...
        request: {
          ...
          method: 'POST',
          responseTimeout: 2,
          url: `https://read.com/save?x-clinia-agent=Clinia%20for%20JavaScript%20(${version})%3B%20Browser`,
        },
        response: {
          content: 'Internal Error',
          isTimedOut: false,
      ...
      },
      {
        ...
        request: {
          ...
          method: 'POST',
          responseTimeout: 2,
          url: `https://read-and-write.com/save?x-clinia-agent=Clinia%20for%20JavaScript%20(${version})%3B%20Browser`,
        },
        response: {
          content: 'Internal Error',
          isTimedOut: false,
    ...
    await expect(transporter.read(transporterRequest)).rejects.toEqual({
      name: 'RetryError',
      message:
        'Unreachable hosts - your application id may be incorrect. If the error persists, contact support@clinia.com.',
