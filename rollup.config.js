...
const clinia = '© Clinia';
const link = 'https://github.com/clinia/clinia-client-javascript';
const createLicence = (name) => `/*! ${name} | ${version} | ${clinia} | ${link} */`;
...
['browser', 'browserLite'].forEach((build) => {
  packagesConfig.push({
    output: build === 'browser' ? 'clinia' : 'clinia-lite',
    package: 'clinia',
...
fs.readdirSync(packagesDir).forEach((dir) => {
  if (dir !== 'clinia' && fs.statSync(path.resolve(packagesDir, dir)).isDirectory()) {
    aliasOptions[`@clinia/${dir}`] = path.resolve(packagesDir, `${dir}/index`);
  }
});
